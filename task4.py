...
In each row, the numers indicate the target number needed out of 20 to land a 
hit on a specific Armor Class, starting with
10 on the left, followed by 9, then 8, all the way to -10 on the far right.

Create a function that reads the specific value for a specific level and an armor class, and 
prints the target number needed.

"""
filename = 'task04.txt'
file = open(filename,'r')
funny = file.read().split("\n")
aclist = []
for i in funny:
        granny = i.split(" ")
        aclist.append(granny)
aclist.pop(-1)
table = aclist
